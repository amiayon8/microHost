...
import { Server, Cpu, HardDrive, User, Plus, LogOut, RefreshCw, Layers } from "lucide-react";
import Link from "next/link";
import { usePathname } from "next/navigation";

function DashboardLayoutContent({ children }: { children: React.ReactNode }) {
  const { user, apps, serverStatus, refreshServerStatus, loading, logout } = useDashboard();
  const pathname = usePathname();

  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center bg-background min-h-screen font-sans">
        <div className="border-4 border-foreground border-t-transparent rounded-full w-10 h-10 animate-spin"></div>
        <p className="mt-4 font-bold text-foreground text-lg">Loading Console...</p>
      </div>
    );
  }

  return (
    <div className="flex flex-col bg-background w-full min-h-screen font-sans">
      <header className="bg-secondary-background shadow-shadow mb-6 border-4 border-border">
        <div className="flex justify-between items-center mx-auto px-6 max-w-7xl h-16">
          <div className="flex items-center gap-2">
            ...
            <span className="font-heading text-2xl tracking-tighter">MICROHOST</span>
          </div>

          <nav className="flex items-center gap-6">
            <Link
              href="/dashboard"
              className={`text-sm font-bold border-b-2 hover:border-foreground transition-all ${pathname === "/dashboard" ? "border-foreground" : "border-transparent"
                }`}
            >
              Dashboard
            </Link>
            <Link
              href="/dashboard/apps"
              className={`text-sm font-bold border-b-2 hover:border-foreground transition-all ${pathname === "/dashboard/apps" ? "border-foreground" : "border-transparent"
                }`}
            >
              Apps
            </Link>
            <Link
              ...
              className={`text-sm font-bold border-b-2 hover:border-foreground transition-all ${pathname === "/dashboard/account" ? "border-foreground" : "border-transparent"
                }`}
            >
              Account
            </Link>
            {user?.is_admin && (
              <Link
                ...
                className={`text-sm font-bold border-b-2 hover:border-foreground transition-all ${pathname === "/dashboard/admin" ? "border-foreground" : "border-transparent"
                  }`}
              >
                Admin
              </Link>
      ...
      <main className="flex-1 mx-auto px-4 w-full max-w-7xl">
        <div className="items-start gap-6 grid grid-cols-1 lg:grid-cols-12">
          <aside className="flex flex-col gap-6 lg:col-span-3 bg-secondary-background shadow-shadow p-6 border-4 border-border">
            <div>
              <h2 className="mb-4 pb-2 border-border border-b-4 font-heading text-lg uppercase">User Status</h2>
            ...
            <div>
              <h2 className="mb-4 pb-2 border-border border-b-4 font-heading text-lg uppercase">My Projects</h2>
              <div className="space-y-3">
                <div className="flex justify-between items-center bg-neutral-100 p-2 border-2 border-border font-bold text-muted-foreground text-xs">
                  ...
                  <span className="bg-foreground px-1.5 py-0.2 rounded-sm text-background">{apps.length}</span>
                </div>
                    ...
                    {apps.slice(0, 5).map((app) => (
                      ...
                      >
                        <div className="flex justify-between items-center font-bold">
                          <span className="max-w-[140px] truncate">{app.filename}</span>
                          <span
                            className={`w-2.5 h-2.5 rounded-full border border-border ${app.is_active ? "bg-green-500" : "bg-red-500"
                              }`}
                          ></span>
                        </div>
                        <div className="text-[10px] text-muted-foreground truncate">{app.id}</div>
                      </div>
                    ))}
                  </div>
                )}
          ...
          <aside className="flex flex-col gap-6 lg:col-span-3 bg-secondary-background shadow-shadow p-6 border-4 border-border">
            ...
            {serverStatus ? (
              <div className="space-y-4">
                <div className="flex items-start gap-3 bg-background p-3 border-2 border-border">
                  ...
                  <div className="flex-1 text-xs">
                    <div className="mb-1 font-bold uppercase">Processor Load</div>
                    <div className="flex justify-between mb-1 text-[11px]">
                      <span>1 min:</span>
                      <span className="font-mono font-bold">{serverStatus.hardware.cpu_load_avg["1m"]}</span>
                    </div>
                    <div className="flex justify-between mb-1 text-[11px]">
                      <span>5 min:</span>
                      <span className="font-mono font-bold">{serverStatus.hardware.cpu_load_avg["5m"]}</span>
                    </div>
                    <div className="flex justify-between text-[11px]">
                      <span>15 min:</span>
                      <span className="font-mono font-bold">{serverStatus.hardware.cpu_load_avg["15m"]}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3 bg-background p-3 border-2 border-border">
                  ...
                  <div className="flex-1 text-xs">
                    <div className="mb-1.5 font-bold uppercase">Memory Status</div>
                    <div className="flex justify-between mb-1 text-[11px]">
                      <span>Usage:</span>
                      <span className="font-mono font-bold">{serverStatus.hardware.ram_usage_percent}%</span>
                    </div>
                    <div className="bg-neutral-200 border border-border w-full h-2">
                      <div
                        className="bg-main h-full"
                        style={{ width: `${serverStatus.hardware.ram_usage_percent}%` }}
                      ></div>
                    </div>
                ...
                <div className="flex items-start gap-3 bg-background p-3 border-2 border-border">
                  ...
                  <div className="flex-1 text-xs">
                    <div className="mb-1.5 font-bold uppercase">Disk Storage</div>
                    <div className="flex justify-between mb-1 text-[11px]">
                      <span>Usage:</span>
                      <span className="font-mono font-bold">{serverStatus.hardware.disk_usage_percent}%</span>
                    </div>
                    <div className="bg-neutral-200 border border-border w-full h-2">
                      <div
                        className="bg-main h-full"
                        style={{ width: `${serverStatus.hardware.disk_usage_percent}%` }}
                      ></div>
                    </div>
                    <div className="mt-1 text-[9px] text-muted-foreground text-right">
              ...
              <div className="py-6 text-sm text-center">Failed to retrieve server data.</div>
            )}
          </aside>
        </div>
      </main>
